...
export default function Login() {
  ...
  const [signInWithEmailAndPassword, user, loading, signInErr] = useSignInWithEmailAndPassword(auth);
  const [createUserWithEmailAndPassword, newUser, creating, signUpErr] = useCreateUserWithEmailAndPassword(auth);
      ...
      if (isLogin) {
        ...
        if (nikExists) {
          ...
          return;
                        ...
                        // Step 1: Buat temporary ID untuk menghubungkan data PostgreSQL dengan Firebase nantinya
                const tempUserId = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2, 15);

                // Step 2: Coba daftarkan ke PostgreSQL dulu
                const internData = {
                    ...form,
                    userId: tempUserId, // Gunakan temporary ID dulu
                    createdAt: new Date().toISOString(),
                    isTemporary: true // Flag untuk menandai ini adalah temporary record
                };
                delete internData.password;

                const res = await fetch("/api/intern/pre-register", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                    },
                ...
                // Step 3: Jika PostgreSQL berhasil, baru daftarkan ke Firebase
                const cred = await createUserWithEmailAndPassword(form.email, form.password);

                if (cred?.user) {
                    // Step 4: Update record di PostgreSQL dengan Firebase UID yang sebenarnya
                    const updateRes = await fetch(`/api/intern/finalize-register/${intern.id}`, {
                        method: "PUT",
                        headers: {
                            "Content-Type": "application/json",
                        },
                    ...
                    if (!updateRes.ok) {
                        // Jika update gagal, hapus user dari Firebase
                        await cred.user.delete();
                        const updateData = await updateRes.json();
                        throw new Error(updateData.message || "Gagal memperbarui data intern.");
                    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-row items-center justify-center gap-3">
      ...
      <img
        ...
        alt="Banner"
        className="w-[55%] h-screen xl:block hidden object-cover"
      />
      <div className="w-full max-w-xl mx-3 my-5">
        <div className="bg-white rounded-3xl shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">{isLogin ? "Magang dan Monitoring Elektronik Terpadu" : "Lengkapi data diri dan periode magang Anda"}</p>
          </div>
              ...
              <>
                <div className="border-t border-gray-200 pt-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-2">Universitas/Kampus</label>
                      <div className="relative">
            ...
            {(signInErr || signUpErr || error) && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-red-700">{error || getFriendlyError(signInErr || signUpErr)}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-600">{isLogin ? "Belum punya akun?" : "Sudah punya akun?"}</p>
        ...
        <p className="text-center text-gray-600 text-sm mt-8">© 2025 BPS Kota Bandar Lampung</p>
      </div>
    </div>
  );
}
...
function Field({ icon, append, ...rest }) {
  return (
    <div className="relative">
      ...
      />
      {append && <div className="absolute inset-y-0 right-0 pr-3 flex items-center">{append}</div>}
    </div>
  );
}
