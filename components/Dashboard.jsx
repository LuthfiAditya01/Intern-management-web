...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        const pembimbing = token.claims.role === "mentor";
        setIsAdmin(admin);
        setIsPembimbing(pembimbing);
        if (token.claims.role === "mentor") {
  ...
  useEffect(() => {
    if (isPembimbing && currentMentorData && interns.length > 0) {
      const activeMentees = interns.filter((intern) => 
        intern.pembimbing?.id === currentMentorData.id && // PERBAIKAN: Gunakan 'id' bukan '_id'
        intern.status === "aktif"
      ).length;
